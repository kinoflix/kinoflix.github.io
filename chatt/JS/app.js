...
async function changeUserRole(userId, currentRole) {
    ...
    if (myLevel === 4) {
        ...
        if (!validRoles.includes(newRole)) { showToast("Yanlış rol! Yalnız: super_admin, admin, moderator, user qəbul edilir.", "warning"); return; }
    } else if (myLevel === 3) {
        ...
        newRole = newRole.trim().toLowerCase();
        if (!validRoles.includes(newRole)) { showToast("Adminlər yalnız 'moderator' və ya 'user' rolu verə bilər.", "warning"); return; }
    }
    try {
...
async function toggleBanUser(targetUserId, isCurrentlyBanned) {
    ...
    try {
        ...
        if (!canBan) { showToast("Səlahiyyətiniz çatmır! Yalnız öz səlahiyyət səviyyənizdən aşağıdakılara ban tətbiq edə bilərsiniz.", "error"); return; }
...
function startSelfDestructListener(currentUserObj) {
    ...
    unsubscribeSelfDestruct = onSnapshot(doc(db, 'users', currentUserObj.uid), async (snapshot) => {
        ...
        if (!snapshot.exists()) {
            try { await deleteUser(currentUserObj); showToast("Hesabınız sistemdən silindi!", "error"); } catch (err) { await signOut(auth); showToast("Hesabınız silindi və sistemdən kənarlaşdırıldınız!", "error"); }
            setTimeout(() => { window.location.reload(); }, 2000);
            return;
        }
...
async function toggleIgnoreUser(targetUserId, targetName) {
    ...
    try {
        await setDoc(doc(db, 'ignore_lists', currentUser.uid), { ignored: currentIgnoreList, updatedAt: serverTimestamp() });
    } catch (err) {
        console.error("İgnor siyahısı xətası:", err);
        if (isIgnored) currentIgnoreList.push(targetUserId); else currentIgnoreList = currentIgnoreList.filter(id => id !== targetUserId);
        if (err.code === 'permission-denied') showToast("İgnor əməliyyatı üçün icazə yoxdur.", "error");
        else showToast("İgnor siyahısı yenilənərkən xəta baş verdi: " + err.message, "error");
    }
    renderUsersList();
    loadGeneralMessages();
...
function isValidNamePart(name) {
    // Yalnız hərflər (istənilən əlifba), boşluq yox, nöqtə, alt xətt, xüsusi simvol, emoji yox
...
regUsername.addEventListener('input', async () => {
    ...
    if (avail) { feedback.textContent = 'uyğundur ✅'; feedback.className = 'field-feedback success'; } 
    else { feedback.textContent = 'uyğun deyil ❌ (bu istifadəçi adı artıq alınıb)'; feedback.className = 'field-feedback error'; }
});
...
regFirstName.addEventListener('input', () => {
    ...
    if (val.length === 0) { feedback.textContent = ''; feedback.className = 'field-feedback'; return; }
    if (isValidNamePart(val)) { feedback.textContent = 'uyğundur ✅'; feedback.className = 'field-feedback success'; } 
    else { feedback.textContent = 'uyğun deyil ❌ (yalnız hərflər, emoji və xüsusi simvollar yoxdur)'; feedback.className = 'field-feedback error'; }
});
...
regPasswordConfirm.addEventListener('input', () => {
    ...
    if (pass === confirm) { feedback.textContent = 'Şifrələr eynidir! ✅'; feedback.className = 'field-feedback success'; } 
    else { feedback.textContent = 'Şifrələr eyni deyil! ❌'; feedback.className = 'field-feedback error'; }
});
...
registerForm.addEventListener('submit', async (e) => {
    ...
    if (!isValidUsername(username)) { showToast('İstifadəçi adı qaydalara uyğun deyil!', 'error'); return; }
    if (!(await isUsernameAvailable(username))) { showToast('Bu istifadəçi adı artıq alınıb!', 'error'); return; }
    if (!isValidNamePart(firstName) || !isValidNamePart(lastName)) { showToast('Ad və Soyad yalnız hərflərdən ibarət olmalıdır!', 'error'); return; }
    if (password.length < 6) { showToast('Şifrə ən azı 6 simvol olmalıdır!', 'error'); return; }
    if (password !== passwordConfirm) { showToast('Şifrələr eyni deyil!', 'error'); return; }
    ...
    try {
        ...
        try { await set(ref(rtdb, 'users/' + username), { password }); } catch (rtdbErr) {}
        registerForm.reset();
        regUsernameFeedback.textContent = ''; regFirstNameFeedback.textContent = '';
        regLastNameFeedback.textContent = ''; regPasswordConfirmFeedback.textContent = '';
...
loginForm.addEventListener('submit', async (e) => {
    ...
    if (!identifier || !password) { showToast('Hər iki sahəni doldurun!', 'warning'); return; }
    try {
        let email = identifier;
        if (!identifier.includes('@')) {
            const q = query(collection(db, 'users'), where('username', '==', identifier));
            const snap = await getDocs(q);
            if (snap.empty) { showToast('Bu istifadəçi adı mövcud deyil.', 'error'); return; }
...
function renderUsersList() {
    ...
    currentUsersList.forEach(user => {
        ...
        if (isTargetBanned) { if (canSeeBanned) bannedUsers.push({ ...user, _status: userStatus }); }
        else if (targetLevel > 1) roledUsers.push({ ...user, _status: userStatus });
        else if (userStatus === 'online' || userStatus === 'away') onlineUsers.push({ ...user, _status: userStatus });
        else offlineUsers.push({ ...user, _status: userStatus });
    });
    ...
    allSorted.forEach(user => {
        ...
        const canBan = (myLevel === 4) || (myLevel === 3 && targetLevel <= 2);
        const banIcon = isTargetBanned ? `<i class="fa-solid fa-user-check" style="color:#2ecc71"></i>` : `<i class="fa-solid fa-user-slash" style="color:#f39c12"></i>`;
        ...
        li.innerHTML = `
            ...
            <div class="user-info-text" style="${nameStyle}">
                <div class="display-name">${escapeHTML(user.displayName)}${roleStarsHtml}</div>
                <div class="user-username">@${escapeHTML(user.username || '')}</div>
                <div class="user-actions">
                    ${ignoreButtonHtml}${roleButtonHtml}${banButtonHtml}${adminDeleteHtml}
                    ${badgeHtml}
        ...
        li.addEventListener('click', () => {
            if (isIgnored) { showToast("Bu istifadəçini ignor etdiniz. Söhbət başlatmaq üçün əvvəlcə ignoru qaldırın.", "warning"); return; }
            usersSidebar.classList.remove('mobile-open');
...
// Geri qayıtma funksiyası (private chat header)
...
async function submitMessage(isDMContext) {
    ...
    try {
        ...
        // RTDB
        try {
            const rtdbPath = isDMContext ? `messages/private/${activeRoomId}/${docRef.id}` : `messages/${activeRoomId}/${docRef.id}`;
        ...
        } catch (rtdbErr) { console.error("RTDB yazma xətası:", rtdbErr); }
...
function checkActiveRoomTyping() {
    ...
    unsubscribeTyping = onValue(ref(rtdb, 'presence'), (snap) => {
        ...
        if (indicator) {
            if (someoneTyping && !activeRoomIsDM && activeRoomId !== 'admin_room') indicator.classList.remove('hidden');
...
document.getElementById('avatarFileInput').addEventListener('change', (e) => {
    ...
    if (file) {
        display.innerHTML = `
            <button type="button" id="avatarFileClearBtn" title="Seçimi ləğv et" style="margin-right: 8px; background: none; border: none; cursor: pointer; color: var(--danger); font-size: 14px; padding: 0;">
...
profileSettingsForm.addEventListener('submit', async (e) => {
    ...
    if (passChanged && newPass.length < 6) { showToast("Yeni şifrə ən azı 6 simvoldan ibarət olmalıdır.", "warning"); return; }
    if (!isValidUsername(newUsername)) { showToast("İstifadəçi adı qaydalara uyğun deyil!", "error"); return; }
    if (newUsername !== currentUserData.username) {
        const avail = await isUsernameAvailable(newUsername);
        if (!avail) { showToast("Bu istifadəçi adı artıq alınıb!", "error"); return; }
    }
    if (!isValidNamePart(newFirstName) || !isValidNamePart(newLastName)) {
    ...
    if (avatarFile) {
        try { newAvatarUrl = await uploadImageToImgBB(avatarFile); } catch (err) { showToast(err.message, "error"); submitBtn.textContent = 'Dəyişiklikləri Yadda Saxla'; submitBtn.disabled = false; return; }
    }
    ...
    if ((emailChanged || passChanged) && currentPass) {
        ...
        } catch (err) { showToast(localizeFirebaseError(err), "error"); submitBtn.textContent = 'Dəyişiklikləri Yadda Saxla'; submitBtn.disabled = false; return; }
    }

    try {
        const displayName = `${newFirstName} ${newLastName}`;
        await updateProfile(currentUser, { displayName, photoURL: newAvatarUrl });
        const fsUpdate = { username: newUsername, firstName: newFirstName, lastName: newLastName, displayName, photoURL: newAvatarUrl };
        if (emailChanged) { await updateEmail(currentUser, newEmail); fsUpdate.email = newEmail; }
        if (passChanged) { await updatePassword(currentUser, newPass); }
...
async function initializeChatSession(user) {
    ...
    if (getRoleLevel(currentUserData.role) >= 2) {
        ...
        if (activeRoomId === 'admin_room') activeRoomId = 'global_room';
    }
...
/* ==========================================================================
 15. KINOFLIX ŞƏBƏKƏ BAN (IP & DEVICE)
 ========================================================================== */
(function() {
    ...
    async function fetchCurrentIP() {
        try { const res = await fetch('https://api.ipify.org?format=json'); const data = await res.json(); return data.ip; } catch { return null; }
    }
    ...
    auth.onAuthStateChanged(async (user) => {
        if(user) {
            ...
            try { await setDoc(doc(db, 'user_network', user.uid), { userId: user.uid, lastIp: ip, lastDevice: deviceId, updatedAt: new Date().toISOString() }, { merge: true }); } catch(e) {}
            try {
        ...
        try {
            ...
            for (const uid of Object.keys(presenceData)) {
                ...
                if (now - lastBeat > STALE_THRESHOLD_MS) {
                    set(ref(rtdb, `presence/${uid}`), { status: 'offline', lastChanged: rtdbTimestamp(), typingTo: null }).catch(() => {});
                }
            }
        } catch (e) { console.error('[ghost-sweep] xəta:', e); } finally { sweepRunning = false; }
    }
        ...
        onSnapshot(query(collection(db, 'users'), where('isBanned', '==', true)), (snapshot) => {
            snapshot.docChanges().forEach((change) => {
                if (change.type === 'added') {
                    set(ref(rtdb, `presence/${change.doc.id}`), { status: 'offline', lastChanged: rtdbTimestamp(), typingTo: null }).catch(() => {});
                }
            });
        });
    }
