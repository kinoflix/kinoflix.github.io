...
function localizeFirebaseError(err) {
    switch(err.code) {
        ...
        default: return 'Xəta baş verdi: ' + err.message;
    }
}

// IMGBB
async function uploadImageToImgBB(file) {
    if (!file.type.startsWith('image/')) throw new Error('Yalnız şəkil yüklənə bilər.');
    const formData = new FormData();
    formData.append('image', file);
    const response = await fetch(`https://api.imgbb.com/1/upload?key=${IMGBB_API_KEY}`, { method: 'POST', body: formData });
    if (!response.ok) throw new Error('Şəkil yüklənərkən xəta baş verdi.');
...
async function deleteAccount() {
    ...
    if (!confirm('Son xəbərdarlıq! Bu əməliyyat geri qaytarıla bilməz!')) return;
    try {
        await deleteDoc(doc(db, 'users', user.uid));
        await deleteUser(user);
        showToast('Hesabınız uğurla silindi.', 'success');
...
async function changeUserRole(userId, currentRole) {
    ...
    if (!validRoles.includes(targetRoleClean)) { showToast('Yanlış rol.', 'warning'); return; }
    try {
        await updateDoc(doc(db, 'users', userId), { role: targetRoleClean });
        showToast('Rol yeniləndi.', 'success');
    } catch (error) { showToast('Xəta baş verdi.', 'error'); }
}
...
async function toggleBanUser(targetUserId, isCurrentlyBanned) {
    ...
    try {
        ...
        if (myLevel === 4 || (myLevel === 3 && targetLevel < 3)) {
            ...
            showToast('Əməliyyat uğurludur!', 'success');
        } else {
            showToast('Səlahiyyətiniz çatmır.', 'error');
        }
    } catch (err) { showToast('Xəta: ' + err.message, 'error'); }
}

async function adminDeleteUser(targetUserId) {
    if (getRoleLevel(currentUserData.role) !== 4) return;
    if (!confirm('Tamamilə silmək istədiyinizə əminsiniz? (Geri qaytarıla bilməz)')) return;
    try {
...
function startSelfDestructListener(currentUserObj) {
    ...
    unsubscribeSelfDestruct = onSnapshot(doc(db, 'users', currentUserObj.uid), async (snapshot) => {
        ...
        if (!snapshot.exists()) {
            try { await deleteUser(currentUserObj); } catch (err) {}
            await signOut(auth);
        ...
        if (snapshot.data()?.isBanned) {
            showToast('Sizin hesabınız ban edildi!', 'error');
            await signOut(auth);
            setTimeout(() => { window.location.reload(); }, 2000);
        }
    });
...
registerForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (!nameSnap.empty) { showToast('Bu istifadəçi adı artıq alınıb.', 'warning'); return; }
...
document.getElementById('googleAuthBtn').addEventListener('click', async () => {
    try {
        ...
        showToast('Google ilə giriş edildi!', 'success');
    } catch (err) { showToast(localizeFirebaseError(err), 'error'); }
});
...
/* ==========================================================================
 6. MOBİL MENU VƏ SİYAHI (Mövcud Məntiq)
 ========================================================================== */
...
function renderUsersList() {
    ...
    currentUsersList.forEach(user => {
        ...
        const roleButtonHtml = (myLevel === 4) ? `<button class="role-toggle-btn" onclick="event.stopPropagation(); changeUserRole('${user.uid}', '${user.role}')"><i class="fa-solid fa-user-gear"></i></button>` : '';
        const canBan = (myLevel === 4) || (myLevel === 3 && targetLevel < 3);
        const banButtonHtml = canBan ? `<button class="admin-ban-btn" onclick="event.stopPropagation(); toggleBanUser('${user.uid}', ${isTargetBanned})"><i class="fa-solid ${isTargetBanned ? 'fa-user-check' : 'fa-user-slash'}" style="color: ${isTargetBanned ? '#2ecc71' : '#f39c12'}"></i></button>` : '';
        const adminDeleteHtml = (myLevel === 4) ? `<button class="admin-user-delete-btn" onclick="event.stopPropagation(); adminDeleteUser('${user.uid}')"><i class="fa-solid fa-user-minus"></i></button>` : '';
        ...
        li.addEventListener('click', () => {
            usersSidebar.classList.remove('mobile-open'); // Mobildə toxunanda sidebarı bağla
...
/* ==========================================================================
 7. OTAQLAR ARASI KEÇİD VƏ MESAJLARIN YÜKLƏNMƏSİ
 ========================================================================== */
// Ümumi otağa qayıdış
...
// Şəxsi başlığa klikləyəndə bağla
...
function closePrivateRoom() {
    ...
    privateChatOverlay.classList.add('hidden'); // Overlayi gizlət
    
    if (unsubscribePrivateMessages) unsubscribePrivateMessages();
    renderUsersList(); // Seçili siyahı elementini təmizlə
}

// Şəxsi (Overlay) Otağı Aç
function openPrivateRoom(targetUser) {
    ...
    // Şəxsi otaq DOM-unu (Overlay) göstər
...
// Ümumi Mesajları Yüklə (Daima işləyir)
...
// Şəxsi Mesajları Yüklə (Yalnız aktiv olduqda)
...
// Element Yaradıcı DOM Funksiyası (Orijinal Kod)
...
// Mesaj Göndərmə Mexanizmi (Hansı layer aktivdirsə oradan oxuyur)
async function submitMessage(isDMContext) {
    ...
    } catch (err) { showToast('Mesaj göndərilərkən xəta!', 'error'); }
}
...
profileSettingsForm.addEventListener('submit', async (e) => {
    ...
    if (newName !== currentUserData.displayName) {
        try {
            ...
            if (nameSnap.docs.some(doc => doc.id !== currentUser.uid)) {
                showToast('Bu ad artıq alınıb.', 'warning');
                submitBtn.innerText = 'Dəyişiklikləri Yadda Saxla'; submitBtn.disabled = false; return;
            }
        } catch (err) { submitBtn.disabled = false; return; }
    }
    if (avatarFile) {
        try { newAvatarUrl = await uploadImageToImgBB(avatarFile); } catch (err) { submitBtn.disabled = false; return; }
    }
    ...
    } catch (err) { showToast('Xəta!', 'error'); } 
    ...
    if (user) {
        ...
        if (userDoc.exists()) {
            ...
            if (currentUserData.isBanned === true) {
                showToast('Giriş əngəlləndi! Hesabınız ban edilib.', 'error');
        ...
        // Daxil olan kimi həmişə arxa fondakı Ümumi Çatı yüklə
        loadGeneralMessages();
        closePrivateRoom(); // Əgər açıq qalan private layihə varsa bağla
