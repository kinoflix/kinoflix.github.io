...
import { doc, updateDoc } from "firebase/firestore"; // Admin et funksiyası üçün
import { 
...
async function changeUserRole(userId, currentRole) {
    // Əksinə çevirmə: admin-dirsə user et, user-dirsə admin et
    ...
    try {
        ...
        showToast(`${newRole === 'admin' ? 'Admin edildi!' : 'İstifadəçi edildi!'}`, "success");
    } catch (error) {
...
function renderUsersList(users, statuses, rooms) {
    ...
    users.forEach(user => {
        ...
        // --- YENİ KODLAR BURADAN BAŞLAYIR ---
        
        // 1. Rol dəyişmə düyməsini hazırla (Yalnız Admin görə bilər)
        const roleButtonHtml = currentUserData.role === "admin" 
            ? `<button class="role-toggle-btn" onclick="event.stopPropagation(); changeUserRole('${user.uid}', '${user.role}')" title="Rolu dəyiş">
                 <i class="fa-solid ${user.role === 'admin' ? 'fa-user-shield' : 'fa-user'}"></i>
               </button>` 
            : '';

        // 2. Silmə düyməsini hazırla (Yalnız Admin görə bilər)
        ...
        // 3. li elementini yarat və innerHTML-i qur
