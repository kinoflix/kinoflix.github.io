...
function renderUsersList(users, statuses, rooms) {
    ...
    users.forEach(user => {
        ...
        // --- TƏLƏB 1: Adı qarşısında Rol İerarxiyasına Görə Sarı Ulduzların Sayı ---
        let adminStarHtml = '';
        if (user.role === "super_admin") {
            adminStarHtml = `<i class="fa-solid fa-star" style="color: #f1c40f; margin-right: 3px; font-size: 11px;" title="Super Admin"></i>`.repeat(3) + ' ';
        } else if (user.role === "admin") {
            adminStarHtml = `<i class="fa-solid fa-star" style="color: #f1c40f; margin-right: 3px; font-size: 11px;" title="Admin"></i>`.repeat(2) + ' ';
        } else if (user.role === "moderator") {
            adminStarHtml = `<i class="fa-solid fa-star" style="color: #f1c40f; margin-right: 3px; font-size: 11px;" title="Moderator"></i>` + ' ';
        }
        ...
        const roleButtonHtml = (myLevel === 4) 
            ? `<button class="role-toggle-btn" onclick="event.stopPropagation(); changeUserRole('${user.uid}', '${user.role || 'user'}')" title="Rolu idarə et (Hazırda: ${user.role || 'user'})">
        ...
        const banButtonHtml = canBan 
            ? `<button class="admin-ban-btn" onclick="event.stopPropagation(); toggleBanUser('${user.uid}', ${isTargetBanned})" title="${isTargetBanned ? 'Banı qaldır' : 'Hesabı banla'}">
                 <i class="fa-solid ${isTargetBanned ? 'fa-user-check' : 'fa-user-slash'}" style="color: ${isTargetBanned ? '#2ecc71' : '#f39c12'}"></i>
               </button>`
            : '';

        // Kökündən Silmə düyməsi (Yalnız Super Admin hər kəsi silə bilər)
        const adminDeleteHtml = (myLevel === 4) 
            ? `<button class="admin-user-delete-btn" onclick="event.stopPropagation(); adminDeleteUser('${user.uid}')" title="İstifadəçini tamamilə sil">
        ...
        li.innerHTML = `
            ...
            <span class="username" style="${nameStyle}">${adminStarHtml}${escapeHTML(user.displayName)}</span>
