...
function renderUsersList() {
    ...
    // Moderatorlar da ban edilmiş istifadəçiləri görə bilsin (temp ban)
    ...
    allSorted.forEach(user => {
        ...
        // Temp ban qaldırma düyməsi (yalnız istifadəçi temp ban edilibsə)
        if (isTargetBanned && user.banExpires) {
            dropdownItems += `<button class="dropdown-item remove-temp-ban-action" data-uid="${user.uid}" data-name="${escapeHTML(username)}"><i class="fa-solid fa-user-check"></i><span class="label">Vaxt banını qaldır</span></button>`;
        }
        ...
        if (myLevel === 4) {
            // IP/Cihaz banı itemi, networkBanned flagına görə yaşıl rəng
    ...
    document.querySelectorAll('.user-actions-dropdown .dropdown-item').forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            } else if (item.classList.contains('remove-temp-ban-action')) {
                const uid = item.dataset.uid;
                const name = item.dataset.name;
                removeTempBan(uid, name);
            }
        });
    });
