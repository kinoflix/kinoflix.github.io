...
/* ==========================================================
  ...
  (function () {
    ...
    // Bütün sayt üzrə CSS animasiya/keçidləri dondurmaq üçün qlobal stil.
    // spidernoir.html ilə müqayisə göstərdi ki, əsas səhifədə arxa planda
    // davamlı işləyən başqa fəaliyyətlər də var — məsələn, chat pəncərəsi
    // ".chat-hidden" ilə yalnız görünməz olur (opacity:0), display:none
    // OLMADIĞI üçün içindəki ".online-dot{animation:pulse 2s infinite}"
    // video modalı açıq olanda da fasiləsiz işləməyə davam edir. Header-in
    // daimi backdrop-filter-i də buna oxşar şəkildə arxa planda aktiv qalır.
    // Bunların hər birini tək-tək ovlamaq əvəzinə, video modalı açıq ikən
    // bütün animasiya/keçidləri toplu şəkildə dayandırırıq.
    const freezeStyle = document.createElement('style');
    freezeStyle.textContent = `
      body.kf-video-freeze * {
    ...
    setInterval(function () {
      ...
      if (open) {
        ...
        // Bütün digər CSS animasiya/keçidlərini dondur (chat pulse və s.)
        document.body.classList.add('kf-video-freeze');
      } else {
        // Video bağlandı — karuseli normal işinə qaytar
        if (typeof startSpotlightAutoplay === 'function') {
          startSpotlightAutoplay();
        }
        document.body.classList.remove('kf-video-freeze');
      }
    }, 400);
  })();
